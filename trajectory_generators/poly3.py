...
class Poly3(TrajectoryGenerator):
    ...
    def generate(self, t):
        ...
        q_dot = (3 * self.a_3 * (t ** 2)) + (self.a_2 * t * (2 - (3 * t))) + (self.a_1 * (1 - (4 * t) + (3 * (t ** 2)))) - (3 * self.a_0 * ((1 - t) ** 2))
        q_ddot = (6 * self.a_3 * t) + (2 * self.a_2 * (1 - (3 * t))) - (2 * self.a_1 * (2 - (3 * t))) + (6 * self.a_0 * (1 - t))

        print(q, q_dot, q_ddot)
